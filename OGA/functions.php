...
function theme_update_scripts() {
    // get_template_directory_uri() is the modern, faster standard over get_bloginfo
    $theme_dir = get_template_directory_uri();
    
    // The 'true' at the end forces the script to load in the footer, removing the render-block!
	wp_enqueue_script("script.js", $theme_dir . '/script.js', array('jquery'), '1.0', true);
	wp_enqueue_script("script.responsive.js", $theme_dir . '/script.responsive.js', array('jquery'), '1.0', true);
}
...
function theme_get_metadata_icons($icons = '', $class = '') {
	...
	for ($i = 0; $i < count($icons); $i++) {
		...
		switch ($icon) {
			...
			case 'date':
				...
				$result[] = '<span class="oga-postdateicon">' . sprintf(__('<span class="%1$s">Published</span> %2$s', THEME_NS),
			...
			case 'author':
				...
				$result[] = '<span class="oga-postauthoricon">' . sprintf(__('<span class="%1$s">By</span> %2$s', THEME_NS),
			...
			case 'category':
				...
				$result[] = '<span class="oga-postcategoryicon">' . sprintf(__('<span class="%1$s">Posted in</span> %2$s', THEME_NS), 'categories', get_the_category_list(', ')) . '</span>';
			...
			case 'tag':
				...
				$result[] = '<span class="oga-posttagicon">' . sprintf(__('<span class="%1$s">Tagged</span> %2$s', THEME_NS), 'tags', $tags_list) . '</span>';
			...
			case 'comments':
				...
				$result[] = '<span class="oga-postcommentsicon">' . theme_ob_get_clean() . '</span>';
			...
			case 'edit':
				...
				$result[] = '<span class="oga-postediticon">' . theme_ob_get_clean() . '</span>';
	...
	return "<div class=\"oga-post{$class}icons oga-metadata-icons\">{$result}</div>";
...
function theme_get_post_thumbnail($args = array()) {
	...
	if ($featured && (has_post_thumbnail())) {
        ...
        // Fetch the real alt text from the media library
        $image_id = get_post_thumbnail_id($post->ID);
        $alt_text = get_post_meta($image_id, '_wp_attachment_image_alt', true);
        if (empty($alt_text)) {
            $alt_text = $title; // Fallback to post title if no alt text exists
        }

        // Force the 'medium' size and inject the correct alt text
		the_post_thumbnail('medium', array('alt' => $alt_text, 'title' => $title));
		...
		if ($attachments) {
            ...
            // Force the 'medium' size for automatic attachments too
			$img = wp_get_attachment_image_src($attachment->ID, 'medium');
			if (isset($img[0])) {
				$result = '<img src="' . $img[0] . '" alt="' . esc_attr($title) . '" width="' . $img[1] . '" height="' . $img[2] . '" title="' . esc_attr($title) . '" class="wp-post-image" />';
			}
		}
	}
	if ($result !== '') {
...
function theme_get_content($args = array()) {
	...
	$more_tag = theme_get_array_value($args, 'more_tag', __('<br /><br />Read More <span class="meta-nav">&rarr;</span>', THEME_NS));
...
function theme_404_content($args = '') {
	...
	$args = wp_parse_args($args, array(
		...
		'focus_script' => '<script type="text/javascript">jQuery(\'div.oga-content input[name="s"]\').focus();</script>'
...
function theme_stylize_pagination($pagination) {
	...
	if ($pagination) {
		...
		$pagination = '<div class="oga-pager">' . str_replace(array('current', 'dots'), array('current active', 'dots more'), $pagination) . '</div>';		
...
function theme_comment_reply_link_filter($link) {
	...
	return str_replace('class=\'', 'class=\'oga-button ', $link);
...
<?php edit_comment_link(__('(Edit)', THEME_NS)); ?></div>
        <div class="oga-comment-content comment-body"><?php if ($comment->comment_approved == '0') : ?>
...
<?php comment_text(); ?></div>
        <div class="oga-comment-footer reply"><?php comment_reply_link(array_merge($args, array('depth' => $depth, 'max_depth' => $args['max_depth']))); ?></div>
    </div>
</div>
